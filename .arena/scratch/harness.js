...
function newClient(name){
  const document = createDom(); const owner = name;
  const winHandlers = {};
  const sb = {
    console, name, document,
    window:{ AudioContext:null, webkitAudioContext:null,
      addEventListener(ev,cb){ (winHandlers[ev]=winHandlers[ev]||[]).push(cb); },
  ...
  sb._pagehide = (persisted) => { (winHandlers['pagehide']||[]).forEach(f=>f({persisted:!!persisted})); };
  return sb;
}
