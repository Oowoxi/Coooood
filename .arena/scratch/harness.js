...
function newClient(name){
  const document = createDom(); const owner = name; const wh = {};
  const sb = {
    console, name, document,
    window:{ AudioContext:null, webkitAudioContext:null, addEventListener(ev,cb){ (wh[ev]=wh[ev]||[]).push(cb); }, location:{ href:'https://x/' } },
    ...
    TextEncoder, Image:function(){ this.onload=null; this.width=1200; this.height=900; Object.defineProperty(this,'src',{set(){ if(this.onload) this.onload(); }}); },
  ...
  sb.$eval = (e)=> vm.runInContext(e, sb);
  ...
  sb._pagehide=(p)=>{ (wh['pagehide']||[]).forEach(f=>f({persisted:!!p})); };
  return sb;
}
