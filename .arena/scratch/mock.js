...
function ref(path, owner){
  ...
  const api = {
    ...
    onDisconnect(){ return { remove(){ dcHooks.push({path,action:'remove',owner}); return Promise.resolve(); },
      set(v){ dcHooks.push({path,action:'set',v,owner}); return Promise.resolve(); },
...
const dcHooks = [];
// محاكاة انطلاق onDisconnect وحده (مثل ما يحصل خطأً عند تبديل التبويب)
function fireOnDisconnectOnly(path){
...
function simulateDisconnect(path){ fireOnDisconnectOnly(path); setPath(path,null); notify(path); }
module.exports = { DB, ref, getPath, setPath, simulateDisconnect, fireOnDisconnectOnly, listeners };
