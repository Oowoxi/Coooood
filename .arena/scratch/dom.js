function createDom(){
  ...
  function makeEl(id, cls){
    const st = { display:'none', setProperty(k,v){ this[k]=v; }, removeProperty(k){ delete this[k]; }, getPropertyValue(k){ return this[k]||''; } };
    const el = {
      id, _html:'', _text:'', value:'', checked:false, _parent:null, _children:[], offsetWidth:100,
      style: new Proxy(st,{ get:(t,k)=> (k in t? t[k] : ''), set:(t,k,v)=>{ t[k]=v; return true; } }),
      ...
      closest(sel){ const w=sel.replace(/^\./,''); let n=this; while(n){ if(n.classList&&n.classList.contains(w)) return n; n=n._parent; } return null; },
      ...
      scrollTop:0, scrollHeight:0, offsetHeight:0, files:[]
    };
    return el;
  }
  const document = {
    _fake:{}, _handlers:{}, visibilityState:'visible',
    getElementById(id){ if(!els[id]) els[id]=makeEl(id); return els[id]; },
    createElement(t){ return makeEl('new-'+t); },
    ...
    addEventListener(ev,cb){ if(ev==='DOMContentLoaded') document._ready=cb; else (document._handlers[ev]=document._handlers[ev]||[]).push(cb); },
    removeEventListener(){},
